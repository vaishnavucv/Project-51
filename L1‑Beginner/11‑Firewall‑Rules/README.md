...
The following verification was performed to ensure the infrastructure meets the requirements.

### Changes Made
- Identified an issue in `entrypoint-B.sh` where the ICMP block rule was incorrectly targeting the `ufw-before-input` chain.
- Updated `entrypoint-B.sh` to block **outgoing** ICMP echo-requests to container A using the `ufw-before-output` chain.

### Verified Test Results
| From | Target | IP | Result | Status |
| :--- | :--- | :--- | :--- | :--- |
| **ufw-A** | **ufw-B** | `172.30.0.3` | Success (0% loss) | ✔ **Passed** |
| **ufw-B** | **ufw-A** | `172.30.0.2` | Blocked (100% loss) | ✔ **Passed** |

#### 1. From A to B (Success)
```bash
